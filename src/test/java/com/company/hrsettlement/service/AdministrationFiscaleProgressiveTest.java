...
class AdministrationFiscaleProgressiveTest {

	private AdministrationFiscale administrationFiscale;

	@BeforeEach
	void initialiser() {
		administrationFiscale = new AdministrationFiscaleProgressive();
	}

	@Test
	@DisplayName("n'impose pas un revenu contenu dans la tranche exoneree")
	void neDoitPasImposerLaTrancheExoneree() {
		BigDecimal impot = administrationFiscale.calculerImpot(
				new AssietteFiscale(new BigDecimal("500000"), BigDecimal.ZERO));
	...
	void doitAppliquerLeTauxDeChaqueTrancheTraversee() {
		...
		BigDecimal impot = administrationFiscale.calculerImpot(
				new AssietteFiscale(new BigDecimal("2000000"), BigDecimal.ZERO));
	...
	void doitAppliquerLeTauxMarginalLePlusEleve() {
		BigDecimal impot = administrationFiscale.calculerImpot(
				new AssietteFiscale(new BigDecimal("10000000"), BigDecimal.ZERO));
	...
	void neDoitJamaisImposerLaPartExoneree() {
		BigDecimal impot = administrationFiscale.calculerImpot(
				new AssietteFiscale(BigDecimal.ZERO, new BigDecimal("5000000")));
